...
class ExamSession:
    ...
    def getuniquequestion(self, qssofar, topic, difficulty="", otherstudents=[], alreadyused=[], qspool={}):
        ...
        while (question in qssofar) \
            ...
            if numtries > 50:
                ...
                if isqsourceinquestions(question.source, alreadyused):
                    print("couldn't find a unique source for " + question.topic + " / " + question.difficulty +
                          " - going to give up and allow overlap with group members")
                elif isuniqueidinquestions(question.uniqueid, otherstudentquestions):
                    print("couldn't find a question not in a group member's exam for " + question.topic +
                          " / " + question.difficulty+" - going to give up and allow overlap with group members")
                break
            question = random.sample(eligibleqs, 1)[0]
            numtries += 1
        ...
        while (question in qssofar) \
            ...
            if numtries2 > 50:
                ...
                if isqsourceinquestions(question.source, alreadyused):
                    print("couldn't find a unique source for " + question.topic + " / " + question.difficulty +
                          " - going to give up and just request unique question instead")
                break
            question = random.sample(eligibleqs, 1)[0]
            numtries2 += 1
        ...
        while (question in qssofar) \
            ...
            if numtries3 > 50:
                ...
                if isuniqueidinquestions(question.uniqueid, alreadyused):
                    print("couldn't find a unique source for " + question.topic + " / " + question.difficulty +
                          " - going to give up and allow repetition of question subtype")
                break
            question = random.sample(eligibleqs, 1)[0]
            numtries3 += 1
        ...
        while (question in qssofar) \
            ...
            if numtries4 > 50:
                ...
                if isuniqueidinquestions(question.uniqueid, alreadyused):
                    print("couldn't find a unique source for " + question.topic + " / " + question.difficulty +
                          " - going to give up and just take whatever question we've got on hand")
    ...
    # Returns a list of Questions comprising one flash exam, or empty if it's not possible,
    #   or None if we didn't succeed but might with further-loosened restrictions on questions
    # Parameters:   topicslist (list of strings): the required topics of the questions
    #               diffslist (list of strings): the required difficulties of the questions
    #               otherstudents (list of student ids): the selected question must not already be
    #                   in any of these students' exams
    #               alreadyseen (list of Questions): the selected question must not be in this list
    #                   (of questions this student has already seen on a previous exam)
    #               qspool (dictionary of topic --> difficulty --> [list of Questions]): questions to draw from -
    #                   should already be date-restricted if applicable
    #               flexibility (integer): how many layers of requirements (on question selection) should be relaxed
    def buildflash(self, topicslist, diffslist, otherstudents=[], alreadyseen=[], qspool={}, flexibility=0):

        qssofar = []

        questions = []
        for t,d in zip(topicslist, diffslist):
            question = self.getuniquequestion_flash(qssofar, t, d, otherstudents, alreadyseen, qspool, flexibility)
            if question is None:  # we've run out of restrictions to loosen
                return []
            elif question is False:  # can't find a question at this level of flexibility
                return None
            else:  # we have a valid question
                questions.append(question)
                qssofar.append(question)
        # got through all the t/d combos successfully
        return questions

    # Returns unique Question with the given topic and difficulty level,
    #   or None if we've run out of restrictions to loosen,
    #   or False if we can't find a question but might with fewer restrictions
    # Parameters:   qssofar (list of Questions): the selected question must not already be in this list
    #               topic (string): the topic from which to collect questions -- must be specified
    #                   (ie, not WILD nor empty)
    #               difficulty (string): the difficulty from which to collect questions;
    ...
    #                   should already be date-restricted if applicable
    #               flexibility (integer): how many layers of requirements (on question selection) should be relaxed
    def getuniquequestion_flash(self, qssofar, topic, difficulty="", otherstudents=[], alreadyused=[], qspool={}, flexibility=0):
        ...
        question = None
        restrs_liftable = []
        while (not restrictions_satisfied) and numtries < 50:
            question = random.sample(eligibleqs, 1)[0]

            # restrictions on selected question...
            # first two *must* be satisfied
            # 1. exact question not already in this exam
            restriction_inexam = question not in qssofar
            # 2. exact question not on another of this student's exams
            restriction_id = not isuniqueidinquestions(question.uniqueid, alreadyused)
            # these three can be lifted; they're listed in reverse order of importance
            # (so the restrictions higher up the page get lifted first)
            # 3. question not any exam of someone this student has worked with
            restriction_group = not isuniqueidinquestions(question.uniqueid, otherstudentquestions)
            # 4. question source not on another of this student's exams
            restriction_source = not isqsourceinquestions(question.source, alreadyused)
            # 5. question subtype not already in this exam
            restriction_subtype = not isqtypeduplicate(question.questiontypes, qtypessofar)

            restrs_liftable = [restriction_group, restriction_source, restriction_subtype]
            if flexibility > len(restrs_liftable):
                return None
            else:
                restrs_liftable = restrs_liftable[flexibility:]

            liftable_satisfied = True
            for restr in restrs_liftable:
                liftable_satisfied = liftable_satisfied and restr
            restrictions_satisfied = restriction_inexam and restriction_id and liftable_satisfied
            numtries += 1
        if restrictions_satisfied:  # then we succeeded before the 50-iteration limit
            return question
        else:  # in 50 iterations we couldn't find a question that satisfied the restrictions
            return False
    ...
    def collectflashquestions(self, sid="", examdate=None):
        ...
        wildcardtopics = [t for t in questionspool.keys() if t != DATASET and t != PHRELAN]
        wildcardtopics = list(set(wildcardtopics))

        # try 50 times to make various combinations of topics/difficulties and form flash exams based on those
        # if we can't, then try again (and again...) each time with one fewer restriction on questions (eg no shared questions with group members, etc)
        questionsforthisexam = None
        flexibility = 0
        while questionsforthisexam is None:
            numattempts = 0  # how many different topic/difficulty combos have we tried?
            while numattempts < 50 and questionsforthisexam is None:
                topicslist, diffslist = maketopicdiffcombo(topicsneeded, diffsneeded, wildcardtopics)
                questionsforthisexam = self.buildflash(topicslist, diffslist, otherstudentsingroup, alreadyseen, questionspool, flexibility)
                numattempts += 1
            if questionsforthisexam is None:
                print("after " + numattempts + ", couldn't generate a flash exam for " + sid + " that satisfies current restrictions; omitting one requirement and trying again")
            # loosen restrictions
            flexibility += 1
        # either we have questions for the exam or an empty list (the latter of which indicates we've run out of restrictions to loosen)
        if len(questionsforthisexam) == 0:
            print("FAIL: couldn't generate a flash exam for" + sid + " even after removing all feasible restrictions; giving up")
