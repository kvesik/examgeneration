...
class ExamSession:
    ...
    #               signups (dictionary of date --> [list of (time,studentid)]): timeslots and corresponding student ids, grouped by date
    ...
    # Parameters:   sid (string): student id whose exam questions to collect (default "": return questions for all students)
    #               examptype (string): examtype whose questions to collect (default "": return questions for all examtypes)
    ...
    def generatelatexexams(self, foldername):
        ...
        for thedate in self.signups.keys():  # should be a date object
            ...
            if (self.examtype == FLASH and thedate <= getfriofthisweek(thedate)) or self.examtype != FLASH:
                ...
                # only use this if you are 100% confident the latex is compilable; otherwise python and xelatex both hang
    ...
    #               topic (string): the topic from which to collect questions -- must be specified (ie, not WILD nor empty)
    ...
    #               alreadyused (list of Questions): the selected question must not be in this list (of questions this student has already seen on a previous exam)
    #               qspool (dictionary of topic --> difficulty --> [list of Questions]): questions to draw from - should already be date-restricted if applicable
    def getuniquequestion(self, qssofar, topic, difficulty="", otherstudents=[], alreadyused=[], qspool={}):
        ...
        for questiontype in question.questiontypes:
            if questiontype in qtypessofar:
        ...
        while (question in qssofar) \
                or questiontypeisduplicate \
                or (isuniqueidinquestions(question.uniqueid, otherstudentquestions)) \
                or (isqsourceinquestions(question.source, alreadyused)):
            if numtries > 50:
                # return None  # TODO this is actually happening sometimes...
                # just give up cause clearly we aren't going to find a unique one...
                return question
            question = random.sample(eligibleqs, 1)[0]
            numtries += 1
        return question
    ...
    def getquestionsbeforestartdate(self, examdate=None):
        ...
        # qsbeforecutoff = [q for q in self.allquestions if makedate(q.datecompleted) <= makedate(self.startdate)]
    ...
    def collectflashquestions(self, sid="", examdate=None):
        ...
        topicslist, diffslist = self.maketopicdiffcombo(topicsneeded, diffsneeded, wildcardtopics)
        while not docombosexist(questionspool, topicslist, diffslist):
            topicslist, diffslist = self.maketopicdiffcombo(topicsneeded, diffsneeded, wildcardtopics)


        # twowildtopics = random.sample(topicsunique, 2)
        # while not docombosexist(questionspool, twowildtopics, [Question.MED, Question.HARD]):
        #     twowildtopics = random.sample(topicsunique, 2)
    ...
    def collectmidtermorfinalquestions(self, extype, sid="", examdate = None):
        ...
        print("student "+sid+" has already seen these questions...", alreadyseen)
        # get list of other students who have worked with this student (could be empty)
        otherstudentsingroup = self.getgroupmembers(sid)
        ...
        for diff in diffsdict.keys():
            for i in range(diffsdict[diff]):
                diffsneeded.append(diff)
        numqs = len(diffsneeded)
        topicsneeded = examcomposition[extype][TOPICS].copy()
        wildcardtopics = examcomposition[extype][WILDTOPICS]
        ...
        questionspool = self.getquestionsbeforestartdate(examdate)
        # questionspool = self.allquestions
    ...
    def maketopicdiffcombo(self, topicsneeded, diffsneeded, wildtopicslist):
    ...
    def writequestionbank(self, texfile, topic, difficulty, questionslist):
        ...
        for idx, question in enumerate(questionslist):
            # texfile.write("~\\\\"+N+N+"{\\large Question "+str(idx+1)+"} - ")
            completedstring = ""
...
def getconfig():
    ...
    return questionspath, signupspath, hasschedule, examtype, examdate, studentgroups
...
#               hasschedule (boolean): True if this exam has scheduled signups (eg 2020W1 flash exams), False if not (eg 2020W1 MT/final)
    ...
    with io.open(signupsfilepath, "r", encoding="utf-8") as sfile:
        ...
        if hasschedule is True:
            ...
            for index, row in df.iterrows():
                ...
                dontuse = False
                # dontuse = dontuse or (stid in existingexams.keys() and examtype in existingexams[stid].keys())
                dontuse = dontuse or (examtype == FLASH and examio.makedate(day) > getfriofthisweek(date.today()))
                if not dontuse:
...
questionspath, signupspath, hasschedule, examtype, examdate, studentgroups = getconfig()
...
numqs = 0
for t in allqs.keys():
    for d in allqs[t].keys():
        numqs += len(allqs[t][d])
print("number of questions: "+str(numqs))

numflashes = 0
nummidterms = 0
for s in existingexams.keys():
    numflashes += len([t for t in existingexams[s].keys() if t == FLASH])
    nummidterms += len([t for t in existingexams[s].keys() if t == MIDTERM])

print("number of existing exams: flash "+str(numflashes)+" / midterm "+str(nummidterms))

print("existing exams: ", existingexams)



# collect scheduling info from file
signups = readsignupsfromfile(signupspath, hasschedule, examtype, examdate)  #, existingexams) #, flashesmade)
...
thisexamsession = ExamSession(examtype, allqs, signups, studentgroups, existingexams, startdate, onepdfperstudent)
