...
Files named `existingexams_donotedit.dict` (plus a generation timestamp). One of these binary files is generated each time you (successfully) run `generateexams.py`. They are not human-readable, but are referenced by the script each time it runs. This is how the script checks to make sure that students don't get the same exam on multiple exams, etc. Note that you will get new .tex files every single time you run the script, even if you've already built those exams once. Their data will simply be read from the biniary file, and the exact same exams will be regenerated. Note that if you delete these files, all exiting data about who has what questions on which exams so far will disappear with it, and you will get *new* random questions for each student upon your next run.
...
* `sid` - student ID
* `day` - date of exam in yyyy-mm-dd format
